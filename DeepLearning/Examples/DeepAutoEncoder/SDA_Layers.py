    ...
    def train(self, layers, X, y, n_iters=1):
        ...
        for iters in range(n_iters):
            ...
            for idx in rows:
                ...
                activations.append(numpy.asarray(tmp[:]).flatten())
                
                grad = layers[-1].backwardPass(y[[idx]])
                for i in range(len(layers)-1, -1, -1):
                    grad = layers[i].update(grad)
            
            errors = X - numpy.asarray(activations)
            sse = ((errors ** 2).sum(1) / X.shape[1]).sum() / X.shape[0]
            rmse = sse ** 0.5
            print "Iteration: {0} RMSE: {1}".format(str(iters), str(rmse))
        return layers
