...
class PerceptronTaggerMultiClass(object):
    ...
    def __init__(self, classes, load=False):
    ...
    def _add_secondary_tag_features(self, feats, word, cls, prev, prev2):
        if prev == self.POSITIVE_CLASS:
            feats["TAG -1 " + cls + ":" + str(prev)]  = 1
        if prev2 == self.POSITIVE_CLASS:
            feats["TAG -2 " + cls + ":" + str(prev2)] = 1
    ...
    def predict(self, essay_feats):
        ...
        for essay_ix, essay in enumerate(essay_feats):
            for sent_ix, taggged_sentence in enumerate(essay.sentences):
                """ Start Sentence """
                class2prev, class2prev2 = {}, {}
                for cls in self.classes:
                    class2prev[cls], class2prev2[cls] = self.START
                ...
                for i, (wd) in enumerate(taggged_sentence):
                    ...
                    for cls in self.classes:
                        ...
                        self._add_tag_features(tagger_feats, wd.word, class2prev[cls], class2prev2[cls])
                        model = self.class2model[cls]
                        guess = model.predict(tagger_feats)

                        class2prev2[cls] = class2prev[cls]
                        class2prev[cls] = guess
            ...
            for essay_ix, essay in enumerate(cp_essay_feats):
                for sent_ix, taggged_sentence in enumerate(essay.sentences):
                    ...
                    for i, (wd) in enumerate(taggged_sentence):
                        ...
                        # get all tagger predictions for previous 2 tags
                        for cls in self.classes:
                            self._add_secondary_tag_features(shared_features, wd.word, cls, class2prev[cls], class2prev2[cls])
                        # train each binary tagger
                        for cls in self.classes:
                            tagger_feats = dict(shared_features.items())
                            # add more in depth features for this tag
