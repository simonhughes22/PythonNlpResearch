...
for i,(essays_TD, essays_VD) in enumerate(folds):
    ...
    top_sent_td_xs = get_sent_feature_for_stacking_from_sentence_model(
        sent_input_feat_tags, sent_input_interaction_tags, sent_td_xs, tag2sent_classifier, SPARSE_SENT_FEATS)

    top_sent_vd_xs = get_sent_feature_for_stacking_from_sentence_model(
        sent_input_feat_tags, sent_input_interaction_tags, sent_vd_xs, tag2sent_classifier, SPARSE_SENT_FEATS)

    print "\nTraining TOP Model"
    tag2top_classifier = train_classifier_per_code(top_sent_td_xs, sent_td_ys_bycode, fn_create_sent_cls, sent_output_train_test_tags)

    td_sent_predictions_by_code \
        = test_classifier_per_code(top_sent_td_xs, tag2top_classifier, sent_output_train_test_tags )

    vd_sent_predictions_by_code \
        = test_classifier_per_code(top_sent_vd_xs, tag2top_classifier, sent_output_train_test_tags )
...
CB_TOP_TD, CB_TOP_VD = "CB_TOP_TD", "CB_TOP_VD"
...
sent_td_objectid = processor.persist_results(CB_TOP_TD, cv_sent_td_ys_by_tag, cv_sent_td_predictions_by_tag, parameters, sent_algo, tagger_id=wd_td_objectid)
sent_vd_objectid = processor.persist_results(CB_TOP_VD, cv_sent_vd_ys_by_tag, cv_sent_vd_predictions_by_tag, parameters, sent_algo, tagger_id=wd_vd_objectid)

print processor.results_to_string(wd_td_objectid,   CB_TAGGING_TD,  wd_vd_objectid,     CB_TAGGING_VD,  "TAGGING")
#print processor.results_to_string(sent_td_objectid, CB_SENT_TD,     sent_vd_objectid,   CB_SENT_VD,     "SENTENCE")
print processor.results_to_string(sent_td_objectid, CB_TOP_TD,     sent_vd_objectid,   CB_TOP_VD,     "SENTENCE")
