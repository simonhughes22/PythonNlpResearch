    ...
    while True:
        ...
        for feat_extractor in all_extractors:
            if feat_extractor.func_name in hs_existing_extractors:
                continue
            macro_f1 = evaluate_feature_set(config, extractors, feat_extractor, features_filename_prefix)
            if macro_f1 > best_macro_f1:
                best_macro_f1 = macro_f1
                f1_improved = True
                new_best_feature_set = feat_extractor
                print(("*" * 8) + " NEW BEST MACRO F1 " + ("*" * 8))
            print("Window Size: " + str(window_size) + "\tFeature Set: " + feat_extractor.func_name.ljust(50) + " attained macro F1: " + str(macro_f1))
...
def evaluate_feature_set(config, existing_extractors, new_extractor, features_filename_prefix):
    ...
    wd_vd_objectid = processor.persist_results(CB_TAGGING_VD, cv_wd_vd_ys_by_tag, cv_wd_vd_predictions_by_tag,
                                               parameters, wd_algo)
    macro_f1 = processor.get_metric(CB_TAGGING_VD, wd_vd_objectid, __MACRO_F1__)
    return macro_f1
...
for win_size in [1, 3, 5, 7, 9]:
    ...
    print("Best macro F1 for window size: " + str(win_size) + " is " + str(macro_f1))
