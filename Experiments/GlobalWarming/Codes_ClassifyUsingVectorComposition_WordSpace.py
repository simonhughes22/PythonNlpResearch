...
class Codes_ClassifyUsingVectorComposition_WordSpace(GwExperimentBase):
...
if __name__ == "__main__":
    ...
    vector_space = ("Lsa", lambda tokenized_docs : lwv.LsaTfIdfSpace(tokenized_docs, topics, normalize=normalize, aggregation_method="sentence"))
    ...
    for parm_val in range(2, 3): #Best C val is 2
        ...
        cl = Codes_ClassifyUsingVectorComposition_WordSpace(topics, func, vector_space[1], algo, (parm_val), use_idf)
        (mean_metrics, wt_mean_metrics) = cl.Run("_VectorSpace_" + vector_space[0] + "_Dims" + str(topics) + "_" + algo + "_" + str(func.func_name) + s_idf + s_params + s_norm + '.txt', stem = stem)
