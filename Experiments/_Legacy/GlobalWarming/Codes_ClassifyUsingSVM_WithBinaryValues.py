...
class Codes_ClassifyUsingSVM_WithBinaryValues(GwExperimentBase):
...
if __name__ == "__main__":
    ...
    for C in range(5, 6):
        cl = Codes_ClassifyUsingSVM_WithBinaryValues(kernel, float(C))
        (mean_metrics, wt_mean_metrics) = cl.Run("Codes_ClassifyUsingSVM_WithBinaryValues_kernel_{0}_c_{1}.txt".format(kernel, str(C)))
