...
class Codes_ClassifyUsingVectorComposition_WithSubSampling_NoThreshold(GwExperimentBase):

    def __init__(self, num_topics, func, vector_space_func, algo, parm_val, use_idf, kernel):
    ...
    def create_classifier(self, code):
        def cls_create(xs, ys):
            ...
            if algo == "SVM":
                classifier = svm.SVC(C = self.parm, probability=False, kernel = self.kernel)
...
if __name__ == "__main__":
    ...
    vector_space = ("Lsa",          lambda tokenized_docs : LatentWordVectors.LsaTermFrequencySpace(tokenized_docs, topics, normalize=normalize))
    #vector_space = ("Lsa",         lambda tokenized_docs : LatentWordVectors.LsaTermFrequencySpace(tokenized_docs, topics, aggregation_method = "sentence", normalize = normalize))
    ...
    for parm_val in range(2, 3): #Best C val is 2
        if algo == "SVM":
            all_params = s_params + str(parm_val)
        else:
            all_params = s_params
        
        cl = Codes_ClassifyUsingVectorComposition_WithSubSampling_NoThreshold(topics, func, vector_space[1], algo, (parm_val), use_idf, kernel)
        (mean_metrics, wt_mean_metrics) = cl.Run("_VectorSpace_" + vector_space[0] + "_Dims" + str(topics) + "_" + algo + "_" + str(func.func_name) + s_idf + all_params + s_norm + '_NoThreshold.txt', stem = stem)
