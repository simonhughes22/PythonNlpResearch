...
""" OUTPUT """
""" SPLIT A """
out_predictions_file_A        = root + "output/predictions_A.txt"
out_predicted_margins_file_A  = root + "output/predicted_confidence_A.txt"
out_metrics_file_A            = root + "output/metrics_A.txt"
out_categories_file_A         = root + "output/categories_A.txt"

""" SPLIT B """
out_predictions_file_B        = root + "output/predictions_B.txt"
...
with open(f_test_essays_A) as f:
    set_test_essays_A = set(map(lambda s: s.strip(), f.readlines()))

with open(f_test_essays_B) as f:
    set_test_essays_B = set(map(lambda s: s.strip(), f.readlines()))

""" Split essays according to lists """
train_essays, train_essay_feats = [], []
test_essays_A, test_essay_feats_A  = [], []
test_essays_B, test_essay_feats_B  = [], []
...
for i, essay in enumerate(tagged_essays):
    ...
    elif essay.name in set_test_essays_A:
        test_essays_A.append(essay)
        test_essay_feats_A.append(feats)
    elif essay.name in set_test_essays_B:
        test_essays_B.append(essay)
        test_essay_feats_B.append(feats)
