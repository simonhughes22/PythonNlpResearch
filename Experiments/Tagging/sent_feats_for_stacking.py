...
def get_sent_feature_for_stacking2(feat_tags, interaction_tags, essays, word_feats, ys_bytag, tag2Classifier, sparse=False, look_back=0):
    ...
    for essay_ix, essay in enumerate(essays):
        ...
        for sent_ix, taggged_sentence in enumerate(essay.sentences):
            ...
            for tag in feat_tags:
                ...
                # number of different instances of the tag sequence (i.e. with gaps)
                # helps a wee bit
                tmp_sentence_xs.append(num_different_tags(pred))
                ...
                yes_no = np.max(pred)
                tmp_sentence_xs.append(yes_no)
                # add total words with tags
                # makes a tiny different
                wds_with_tag = np.sum(pred[pred > 0])
                tmp_sentence_xs.append(wds_with_tag / float(ys.shape[0]))

                if yes_no > 0.0:
                    un_pred_tags.add(tag)

                    first_ixs[np.argmax(pred)].append(tag)
                    # for adding 2-way feature combos

            sk = sorted(first_ixs.keys())
            processed = []
            # a->b
            seq_pairs = set()
            for key in sk:
